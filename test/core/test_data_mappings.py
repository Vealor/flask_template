...
class TestDataMappingsGet():
    ...
    #     response = get_req('/core/data_mappings?caps_gen_id=' + str(id), client, token)
    #
    ...
    def test_list_caps_gen_fail(self, api, client):
        token = login(client, 'lh-admin', 'Kpmg1234%')
        response = get_req('/core/data_mappings', client, token)
    ...
    def test_list_single_fail(self, api, client):
        token = login(client, 'lh-admin', 'Kpmg1234%')
        response = get_req('/core/data_mappings/-1', client, token)
