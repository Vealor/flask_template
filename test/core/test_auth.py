...
class TestAuthReset():
    def test_reset_success(self, api, client):
        ...
        new_user = post_req('/core/users', client, {
        ...
        del_new_user = delete_req('/core/users/' + str(new_user_id), client, helper_token)
...
class TestAuthLogin():
    ...
    def test_login_inactive(self, api, client):
        ...
        assert new_user.status_code == 201
        new_user_id = (new_user.get_json())['payload'][0]['id']
        deactivate = put_req('/core/users/' + str(new_user_id) + '/deactivate', client, token=helper_token)
