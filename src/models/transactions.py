...
class Transaction(db.Model):
    ...
    def predictive_serialize(self):
        ...
        if self.transaction_project.has_ts_gst and self.gst_signed_off_by_id:
            output['codes']['gst'] = [c.transaction_gst_code_code.code_number for c in self.gst_codes]
        if self.transaction_project.has_ts_hst and self.hst_signed_off_by_id:
            output['codes']['hst'] = [c.transaction_hst_code_code.code_number for c in self.hst_codes]
        if self.transaction_project.has_ts_qst and self.qst_signed_off_by_id:
            output['codes']['qst'] = [c.transaction_qst_code_code.code_number for c in self.qst_codes]
        if self.transaction_project.has_ts_pst and self.pst_signed_off_by_id:
            output['codes']['pst'] = [c.transaction_pst_code_code.code_number for c in self.pst_codes]
        if self.transaction_project.has_ts_apo and self.apo_signed_off_by_id:
            output['codes']['apo'] = [c.transaction_apo_code_code.code_number for c in self.apo_codes]
        return output
